...
SELECT id, created_at, updated_at, title, url, description, published_at, feed_id FROM posts LIMIT 10
`
...
const getPostsUrls = `-- name: GetPostsUrls :many
SELECT url FROM posts
`

func (q *Queries) GetPostsUrls(ctx context.Context) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, getPostsUrls)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
